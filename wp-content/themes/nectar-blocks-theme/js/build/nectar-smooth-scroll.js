...
function() {
    var t = !(!window.nectarOptions || !window.nectarOptions.smooth_scroll || "true" !== window.nectarOptions.smooth_scroll),
        i = window.nectarOptions && window.nectarOptions.smooth_scroll_strength ? parseFloat(window.nectarOptions.smooth_scroll_strength) : .85,
        e = navigator.userAgent.match(/(iPod|iPhone|iPad)/) || navigator.userAgent.match(/Mac/) && navigator.maxTouchPoints && navigator.maxTouchPoints > 2,
        s = (navigator.userAgent.indexOf("Safari") > -1 && navigator.userAgent.indexOf("Chrome"), navigator.userAgent.match(/Android|webOS|iPhone|iPad|iPod|BlackBerry|Windows Phone/i));
    if (!e && !s && t) {
        const t = new Lenis({
            lerp: .33 - i / 4,
            prevent: t => t.classList.contains("nectar-modal") || t.classList.contains("nectar-allow-scroll")
        });
        t.on("scroll", (t => {
            window.ScrollTrigger && ScrollTrigger.update(), window.nectarDOMInfo && (window.nectarDOMInfo.scrollTop = t.animatedScroll)
